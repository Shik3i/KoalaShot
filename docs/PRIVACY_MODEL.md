...
The original PNG stays in popup memory until the requested action is complete. If editor handoff is enabled, one Blob is stored in the extension origin's IndexedDB under a cryptographically random ID. The editor stores only a validated, temporary annotation draft alongside that Blob using debounced local writes. Records and drafts are pruned on popup/editor start and expire after 24 hours. There is no history view, synchronization, web-accessible storage, query-string image data, data URL editor, or remote editor.
