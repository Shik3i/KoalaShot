...
On 2026-08-20, the v0.3.2 `npm test` release gate passed 30/30 unit tests, ESLint, production dependency audit with 0 vulnerabilities, both extension builds, source/archive/immutable-workflow validation, and AMO validation with 0 errors, 0 notices, and 0 warnings. The real Chrome/Firefox success and permission-denial browser matrix also passed.
...
- Firefox passed success and permission-denial WebDriver BiDi flows against the v0.3.2 temporary test archive on 2026-08-20, including real popup/editor clipboard success, exact-PNG denial fallback, all Chrome flow coverage above, the split capture/draft IndexedDB schema, and 390×844 responsive geometry.
...
On 2026-08-20 the built landing page additionally passed visual 1280×900 and 390×844 checks in light and dark color schemes. The original 720×749 2D mascot loaded at both breakpoints, desktop `scrollWidth === 1280`, mobile `scrollWidth === 390`, and neither layout had horizontal overflow.
