...
- The editor stores temporary captures and validated annotation/crop drafts in separate extension-local IndexedDB stores. A tab-scoped `sessionStorage` journal protects the latest validated draft until IndexedDB persistence completes. Records become inaccessible after 24 hours; an open editor deletes at the deadline, startup prunes expired records, and explicit discard deletes immediately.
