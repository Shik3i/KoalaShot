...
- `Capture & edit` opens the local editor without an original download or clipboard write.
...
- Editor tools are Select, Pan, Pen, Highlighter, Arrow, Line, Rectangle, Ellipse, Text, secure opaque Redact, cosmetic Pixelate, cosmetic Blur, numbered Marker, Crop, Undo, Redo, Delete, Clear all, zoom, fit width, and actual size.
- The editor stores temporary captures and validated annotation/crop drafts in extension-local IndexedDB, with a tab-scoped sessionStorage journal, bounded expiry and explicit cross-tab discard.
...
- Landing pages ship a strict document CSP and a `_headers` contract for hosts that support response-header files; production hosts must apply the same security headers.
- Publisher details are maintained through the central KoalaStuff legal notice, linked from the project legal page.
