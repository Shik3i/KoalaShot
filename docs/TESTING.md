...
The matrix command runs success and permission-denial flows for both browsers. CI and tagged releases pin the currently qualified Chrome-for-Testing `151.0.7922.34` and Firefox `152.0.3`; browser-version updates require a successful matrix run before changing those pins. On macOS and Linux the Chrome harness discovers Playwright Chrome-for-Testing caches as well as installed browsers; environment overrides remain available.
