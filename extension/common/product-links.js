...
export const SUPPORT_URL = "https://shot.koalastuff.net/help/";
...
export async function initializeFooter() {
  ...
  const brand = document.createElement("strong"); brand.textContent = t("ui_koalashot");
  const version = document.createElement("span");
  identity.append(brand, version);
  const note = document.createElement("span"); note.textContent = t("ui_local_capture_no_uploads");
  identity.append(note);
  const links = document.createElement("nav"); links.setAttribute("aria-label", t("ui_koalashot_support"));
  for (const [key, href, hint] of [["ui_privacy", PRIVACY_URL, "ui_read_privacy"], ["ui_legal", LEGAL_URL, "ui_read_legal"], ["ui_help", SUPPORT_URL, "ui_read_help"]]) {
    const label = t(key);
  ...
  rating.textContent = t("ui_reviews_after_store_launch"); links.append(rating);
  ...
  try {
    ...
    if (href) {
      const link = document.createElement("a"); link.textContent = t("ui_rate"); link.title = t("ui_rate_hint");
